...
@borrow_router.post("/", response_model=BorrowScheme, responses={
...
@borrow_router.get("/", response_model=list[BorrowScheme], responses={
...
@borrow_router.get("/{id}", response_model=BorrowScheme, responses={
...
@borrow_router.patch("/{id}/response", response_model=BorrowUpdateScheme, responses={
        404: {"description": "Запись о выдаче по указанному айди не найдена."},
...
async def api_finished_borrow(id: int, return_date: date, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    if borrow is None:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Книга уже сдана.")
    elif borrow == "Invalid return_date":
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="Дата возврата не может быть раньше даты взятия книги."
